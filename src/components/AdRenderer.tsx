...
import { displayText, elementStyle } from '../rendering/renderDom';

export function AdRenderer({ layout, ad }: { layout: ResolvedLayout; ad: AdSpecification }) {
  ...
  useLayoutEffect(() => {
    ...
    const updateScale = () => {
      ...
      const next = Math.min(availableWidth / layout.width, availableHeight / layout.height);
  ...
  return (
    <div ref={stageRef} className="ad-render-stage" data-preview-scale={scale}>
      <div
        ...
        data-render-width={previewWidth}
        data-render-height={previewHeight}
      >
        <div
          className="ad-frame"
          style={{
          ...
          {[...layout.elements]
            ...
            .map((e) => {
              ...
              if (e.type === 'image' && e.role !== 'branding') {
                return (
                  <div key={e.id} className="ad-element image" data-element={e.id} style={common}>
                    <img
                      src={s.content}
                      alt={s.role}
                      draggable={false}
              ...
              if (e.type === 'image' && e.role === 'branding') {
                return (
                  <div
                    ...
                    data-element={e.id}
                    data-truncated={e.truncated ? 'true' : 'false'}
                    data-repositioned={e.repositioned ? 'true' : 'false'}
                    style={{
                      ...common,
                      fontSize: e.fontSize,
                      lineHeight: e.lineHeight,
                      whiteSpace: e.truncated ? 'nowrap' : 'normal',
              ...
              if (e.type === 'button') {
                return (
                  <button key={e.id} className="ad-element cta" data-element={e.id} style={common}>
                    {s.content}
                  </button>
                );
              }
