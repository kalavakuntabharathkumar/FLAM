...
export interface DegradeState { hidden:Set<string>; trunc:Set<string>; repositioned:Set<string>; repositionAttempted:Set<string>; }
...
export function nextReposition(specs:AdElementSpec[],state:DegradeState){
 ...
 for(const p of priorities){
  ...
  const candidate=atThisPriority.find(e=>
  ...
  if(candidate){return {elementId:candidate.id,reason:`Repositioned priority ${p} content to a free safe-area corner instead of dropping it.`};}
 }
 return null;
}
